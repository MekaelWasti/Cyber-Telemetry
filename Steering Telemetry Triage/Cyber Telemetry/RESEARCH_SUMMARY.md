...
## Extension experiments: one lever helps, the rest don't

We stress-tested five ways to *improve* the steering geometry. Only analyst feedback on the
**query side** helped; every attempt to push more information *into the embedding* failed to beat
the lean baseline — each with a shuffled / no-op control proving it is a real effect, not a bug.
(Baseline recall@100 = 0.512; all variants are 3-seed consensus, leave-one-anchor-out over 19 anchors.)

| Extension | Idea | Δ recall@100 vs baseline | Verdict |
|---|---|---|---|
| Iterative **query-side** feedback | confirm hits → re-aim the query (centroid / Rocchio); space fixed | **+0.032**, paired p = 0.004 | **helps** |
| Embedding-side re-metric | re-fit an LDA / Mahalanobis metric from confirmations | −0.018 (p ≈ 1) | overfits |
| LLM analyst descriptions → re-embed | per-session blind CoT as node features | −0.012 (p = 0.43); **oracle ceiling also flat** | no help |
| Richer edges | `shares_dll` / `args` / `path` + extended same-user | −0.050 (1/19), p ≈ 0.99; shuffled collapses | no help |
| Temporal node encoding | multi-scale time + session position on nodes | −0.219 (0/19), p ≈ 1 | **hurts** |
| Temporal edge-time (TGAT) | functional Δt on edges via attention (`TransformerConv`) | ±0.000 vs no-time control; −0.029 vs SAGE | **inert** |

**The convergent finding.** Across all five "add information to the embedding" experiments the pattern
is identical: extra signal tightens the *immediate* malicious neighborhood (separation gap often rises)
but does **not** extend top-100 retrieval. Temporal modeling fails two distinct ways: **on the nodes**
absolute time actively hurts (the model reorganizes the geometry around *time* — loss drops, retrieval
craters), and **on the edges** (TGAT/`TransformerConv` with functional Δt) it is completely **inert** —
identical to the no-time control — because the relations are *already built with time windows*, so Δt is
redundant. The mechanism is the data: the ACME4 red team runs **stock Windows LOLBins with benign command lines**
(`cleanmgr /autocleanstoragesense`, `rundll32 startupscan.dll,susruntask`), per-run-unique `dismhost`
GUID paths, and activity **spread across the slice** — so it is behaviorally *and* temporally
camouflaged inside benign power-user activity (`baduser3` 89 % LOLBin sessions vs. benign `davidf` 85 %,
`grantj` 80 %). The only true discriminator is **identity** (one user on one host), which the baseline
same-user edges already capture; everything else is redundant with that or distracting.

A useful sub-result: a **blind LLM analyst** (CoT over processes + edges + cross-session pattern) flags
the campaign at **recall 1.00 but precision 0.42** — it catches every malicious session yet cannot avoid
benign LOLBin false positives, which is exactly why richer features don't separate the cluster.

**What this means (a strength, not a weakness):** the simple real-edge graph + one-anchor steering —
optionally with query-side relevance feedback — is the right configuration, and we can say so *with
controls* against richer alternatives. These negatives also retire the original "temporal node encodings
improve grouping" criterion: on this data they don't — neither node-time (hurts) nor edge-time attention
(inert). Richer edges / descriptions / temporal modeling are worth revisiting only on a dataset where malware
is behaviorally or temporally **distinct** from benign activity.
